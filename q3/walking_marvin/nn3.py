...
class NeuralNet3:
 ...
 #           action[i] = self.sum_w_b(actor.W[i], actor.B[i], inputs)
    ...
    #takes 144W, 72B, 24S.
    def net(self, actorW, actorB, inputs):
        W, B = self.hack_dropout(actorW, actorB)
        ...
        DATA = self.compute_combination(W[132:151], B[72:78], Li_data, Leg_data, inputs[5:14])
        return self.output_layer(W[0:60], B[0:28], DATA)

    def hack_dropout(self, actorW, actorB, dropout=0.1):
        W = actorW.copy()
        B = actorB.copy()
        for i in range(len(W)):
            x = rand.randint(0, 1 // dropout - 1)
            if x == 0:
                W[i] = 0
    ...
    #takes 19W, 6B, 3Li, 4Lg, 4S (across 8 signals, contacts and knee angles)
    def compute_combination(self, W, B, Li, Lg, S):
        if S[7] == 0:
            ...
            J2 = self.relu_sum(W[16:18], B[4:6], S[0])
            D2 = tanh(Lg[1] * W[12] + Li[0] * W[13] + Li[1] * W[14] + Li[2] * W[15] + J2 * W[18] + B[3])
        if S[2] == 0:
            ...
            J4 = self.relu_sum(W[16:18], B[4:6], S[5])
            D4 = tanh(Lg[3] * W[12] + Li[0] * W[13] + Li[1] * W[14] + Li[2] * W[15] + J4 * W[18] + B[3])
        return D1, D2, D3, D4
    ...
    def compute_legs(self, W, B, S, H):
        ...
        if S[9] == 0:
            ...
            L1 = tanh(L1 + S[4] * self.relu(L1 * W[58] + B[30]) + H[1] * W[60] + B[32])
            L2 = tanh(L2 + S[4] * self.relu(L2 * W[59] + B[31]) + H[1] * W[61] + B[33])
        if S[4] == 0:
            L3 = tanh(L3 + S[9] * self.relu(L3 * W[56] + B[28]))
            L4 = tanh(L4 + S[9] * self.relu(L4 * W[57] + B[29]))
        else:
            L3 = tanh(L3 + S[9] * self.relu(L3 * W[58] + B[30]) + H[3] * W[62] + B[34])
            L4 = tanh(L4 + S[9] * self.relu(L4 * W[59] + B[31]) + H[3] * W[63] + B[35])
        return L1, L2, L3, L4
